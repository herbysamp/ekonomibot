...
if (author !== null && timeout - (Date.now() - author) > 0) {
    ...
    .setColor("#FFFFFF")
    .setDescription(`:cross: Anda sudah merampok seseorang\n\nLakukan lagi dalam **waktu${time.minutes}menit ${time.seconds}detik**`);
    message.channel.send(timeEmbed)
  } else {

let moneyEmbed = new Discord.RichEmbed()
  .setColor("#FFFFFF")
  .setDescription(`:cross: Anda membutuhkan setidaknya Rp.200 di dompet Anda untuk merampok seseorang `);
...
let moneyEmbed2 = new Discord.RichEmbed()
  .setColor("#FFFFFF")
  .setDescription(`:cross: ${user.user.username} tidak memiliki apa pun yang bisa Anda rampok`);
if (targetuser < 0) {
...
.setDescription(`<:Check:618736570337591296> Anda merampok ${user} dan dapat Rp.${random} `)
.setColor("#FFFFFF")
